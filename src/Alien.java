...
/**
 * This class represents an Alien with three circles! When created, its position
 * and
 * color is randomized using the randomLocGenerator() and randomColorGenerator
 * methods().
 * The Alien's location and color can be changed using its mutator methods of
 * setXPos(),
 * setYPos(), and setColor(). Additionally, the Alien can be removed from the
 * pane with
...
public class Alien {
    ...
    /**
     * The constructor takes in a pane and assigns it to the instance variable
     * this.alienPane.
     * It's x and y location are calculated randomly as well as its color, and three
     * circles
     * are created using the generated location and color (the aliens body, left
     * eye, and right
    ...
    public Alien(Pane alienPane) {
        ...
        this.leftEye = new Circle(xPos - Constants.EYE_X_OFFSET, yPos - Constants.EYE_Y_OFFSET, Constants.EYE_RADIUS,
                Color.BLACK);
        this.rightEye = new Circle(xPos + Constants.EYE_X_OFFSET, yPos - Constants.EYE_Y_OFFSET, Constants.EYE_RADIUS,
                Color.BLACK);
